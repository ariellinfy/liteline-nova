...
CREATE TABLE IF NOT EXISTS room_memberships (
  ...
  CONSTRAINT id PRIMARY KEY (user_id, room_id)
);
...
-- 1) USERS
-- Authenticate & lookups by email/username (you already have these; keep them)
...
-- (Optional) if you often list rooms by creator:
CREATE INDEX IF NOT EXISTS idx_rooms_created_by
  ON rooms (created_by);

-- 3) ROOM_MEMBERSHIPS
-- Fast count of active members per room (JOIN + COUNT where is_active = true)
-- and fast ORDER BY rm.joined_at in getRoomMembers
...
-- 4) MESSAGES
-- Message pagination & latest-first queries:
--   WHERE room_id = $1
--   AND (optionally) created_at < $cursor
--   ORDER BY created_at DESC
--   LIMIT $n
-- This composite index supports both the filter and the sort.
